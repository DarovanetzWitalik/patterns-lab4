...
import lab4.lighthtml.ClosingType;
import lab4.lighthtml.DisplayType;
import lab4.lighthtml.LightElementNode;
import lab4.lighthtml.LightTextNode;
import lab4.lighthtml.RecordingEventContext;
...
public class Main {
    public static void main(String[] args) {
        ...
        System.out.println(runObserverDemo());
    ...
    private static String runObserverDemo() {
        StringBuilder sb = new StringBuilder();
        sb.append("=== Спостерігач: події LightHTML ===").append(System.lineSeparator());

        LightElementNode button = new LightElementNode("button", DisplayType.INLINE, ClosingType.PAIR)
                .addClass("btn")
                .addClass("btn-primary")
                .addChild(new LightTextNode("Зберегти"));

        button.on("click", (event, source) -> RecordingEventContext.record(
                "Обробник 1: натиснуто елемент <" + source.getTagName() + ">"));
        button.on("click", (event, source) -> RecordingEventContext.record(
                "Обробник 2: запускаємо валідацію перед збереженням."));
        button.on("mouseover", (event, source) -> RecordingEventContext.record(
                "Підказка: кнопка зберігає поточний документ."));

        sb.append(button.outerHTML()).append(System.lineSeparator());
        sb.append(button.dispatchEvent("click"));
        sb.append(button.dispatchEvent("mouseover"));
        return sb.toString();
    }
